...
cjdic = {
    ...
    'GKCJX10': ['技术'],
    'GKCJX11': [
        '统考美术成绩', '美术专业分', '美术总分', '艺体专业成绩', '美术统考总分', '美术与设计学类', '美术本科统考成绩',
        '美术'
    ],
...
zyml = pd.read_excel(
    r"C:\Users\zym\Desktop\资料\招生办\2020招办\数据库处理方法\2020专业学院名称.xlsx")
for i in zyml.index:
    zydic[zyml['招生专业目录'].at[i]] = zyml['所属学院'].at[i]

# p1 = input("输入源数据所在文件夹路径 分隔符为/ :")
# p2 = input("导出文件或已导出一部分数据的文件地址(最后加上文件名) 分隔符为/ :")
p1 = "C:/Users/zym/Desktop/资料/招生办/2020招办/操作文件夹/总数据表/测试/源数据/"
p2 = "C:/Users/zym/Desktop/资料/招生办/2020招办/操作文件夹/总数据表/测试/result.xlsx"
List = file_dirs(p1)
way = 1
if way == 1:
    result = DT.iloc[1:, :]
...
for x in List:
    ...
    if "T_BMK.dbf" in FilesList:
        ...
        table = DBF(os.path.join(path, "T_TDD.dbf"), encoding="GBK")
        df = pd.DataFrame(iter(table))

    cjdm = DBF(os.path.join(path, "TD_CJXDM.dbf"), encoding="GBK")

    cjdmsm = pd.DataFrame(iter(cjdm))
    # pcdmdbf = DBF(os.path.join(path, "TD_PCDM.dbf"), encoding="GBK")
    # pcdm = pd.DataFrame(iter(pcdmdbf))
    # kldmdbf = DBF(os.path.join(path, "TD_KLDM.dbf"), encoding="GBK")
    ...
    for i in cjdmsm.index:
        dic2["GKCJX" + cjdmsm["CJXDM"].at[i]] = cjdmsm["CJXMC"].at[i].replace(
            u'\u3000', u'')
    # pcdic = {}
    # for i in pcdm.index:
    ...
    dv = pd.merge(dv, tddxx[['KSH', 'LQZY']], on='KSH')
    ...
    for i in re.iloc[1:, :].index:
        ...
        if '(' in LQZY and '定向' not in LQZY and '中外合作办学' not in LQZY:
            # re['LQZY'].at[i]=re['LQZY'].at[i][1:re['LQZY'].at[i].index('(')]
            if '1' <= LQZY[0] <= '6' or LQZY[0] == ' ':
                # re['XY'].at[i] = zydic[LQZY[1:LQZY.index('(')]]
                re['LQZY'].at[i] = LQZY[1:]
            # else:
            #     re['XY'].at[i] = zydic[LQZY[0:LQZY.index('(')]]

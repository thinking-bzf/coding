...
class BPNN:
    def __init__(self, num_in, num_hidden, num_out):
        ...
        #创建权重矩阵
        self.w_in = np.random.rand(self.num_in - 1, self.num_hidden - 1)
        self.w_out = np.random.rand(self.num_hidden - 1, self.num_out)
        ...
        #最后建立动量因子（矩阵）
        self.i = np.zeros([self.num_in - 1, self.num_hidden - 1])
        self.o = np.zeros([self.num_hidden - 1, self.num_out])
        self.ci = makematrix(self.num_in, self.num_hidden)
        self.co = makematrix(self.num_hidden, self.num_out)

    #信号正向传播
    def update(self, inputs):
        if inputs.shape[0] != self.num_in - 1:
        ...
        for i in range(self.num_hidden - 1):
            ...
            sum += np.dot(np.array(self.active_in[i]), self.w_in[i]).sum()
            self.active_hidden[i] = sigmoid(
                sum)  #active_hidden[]是处理完输入数据之后存储，作为输出层的输入数据
        ...
        for i in range(self.num_out):
            ...
            sum += np.dot(np.array(self.active_hidden[i]), self.w_out[i]).sum()
    ...
    def errorbackpropagate(self, targets, lr, m):  #lr是学习率， m是动量因子
        ...
        for i in range(self.num_hidden):
            ...
            error += (np.array(out_deltas) * self.w_out[i]).sum()
        ...
        for i in range(self.num_hidden):
            for j in range(self.num_out):
                ...
                self.wight_out[i][
                    j] = self.wight_out[i][j] + lr * change + m * self.co[i][j]
                self.co[i][j] = change
        ...
        for i in range(self.num_in):
            for j in range(self.num_hidden):
                ...
                self.wight_in[i][
                    j] = self.wight_in[i][j] + lr * change + m * self.ci[i][j]
                self.ci[i][j] = change
    ...
    def train(self, pattern, itera=20000, lr=0.1, m=0.1):
